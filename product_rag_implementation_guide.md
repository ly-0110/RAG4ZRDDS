...
- [ ] PDF 清洗：PyMuPDF(fitz) 逐页提取文本，剥离固定页眉“臻融数据分发服务DDS 系统软件”与页码行，同时记录物理页码与印刷页码（相差 7，2026-08-28 实测会签定值）；产物 `data/cleaned/pages.jsonl`
...
- 数据质量点：固定页眉待剥离；表格会被线性化；150 张截图图片分布在 29 页（日志示例多为图）；印刷页码比物理页码大 7（2026-08-28 实测会签定值，旧记录 6 有误）；原文有笔误（如 DomainParticipnt），可作检索测试素材
