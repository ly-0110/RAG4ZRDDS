# RAG4ZRDDS API 契约（v0.6 · score 量纲澄清 D）
...
> **score 阈值待会签（2026-09-07 提出）**：任何基于 score 的判定（如 Citation 会签第 5 题的「弱证据」标记）都必须按 `retrieval.mode` 分别定标。单一阈值 0.5 在 `vector` 下会把约一半正常命中误判为弱证据，在 `bm25` 下则永不触发（静默失效）。`bm25` 模式下 score 恒 > 0（零词面重叠的候选已在检索侧过滤），故「返回空 sources」才是该模式唯一的无证据硬信号。字段形状未变，前端解析无需改动。
