...
> **v0.9（2026-09-17）**：§3「通路切换」里"`hybrid_rerank` 因精排阻塞事件循环仍不建议挂入演示动线"是 v0.3 的旧口径，与 §4 缺口 2（v0.5 已改"可进演示动线"）自相矛盾——本次统一为可挂动线 + 首切预热口径。
...
**通路切换（v0.4 起，无需重启）**：各场景的"配置"行记录的是该场景**实测时的启动配置**；现在同一服务内可直接切换——`/query` 请求体带 `"experiment": "<实验ID>"`（如 `struct_multisrc_v1`、`struct_multisrc_hybrid`），白名单 = `/healthz` 返回的 `experiments`。注意：**首次切换到某实验需现场装载模型/索引（数秒~数十秒），演示前先把要用到的实验各点一题预热**；`hybrid_rerank` 亦可挂入动线（精排打分已随 B 的 PR#40 移出事件循环，服务不再冻结），只是首切要等模型冷加载，同样按"先点一题预热"处理——口径见 §4 缺口 2。
