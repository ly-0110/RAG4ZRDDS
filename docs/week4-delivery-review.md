...
**当前状态（2026-09-15，合并全部 PR 后）**：`pytest tests/` **345/345 绿**；六套索引指纹匹配、合并全程未触发重建。`make audit` 现为 **blocked**——六题题干回退原文后回到实体零命中的已知真阳性（循环指纹 4/120），待 E 重写题干或 C 转拒答口径后重开指标闸门（§3.4 P0）。
...
| 级别 | 问题 | 处置 |
|---|---|---|
| **P0** | Q021/Q023/Q028/Q059/Q060/Q119 六题题干经 E 重写（意图为消除零命中实体），但写入环节发生有损转码——非 ASCII 字符全部变字面 `?`、ASCII 完好，文本已废；乱码题干会污染检索与计分 | 题干已回退至原文（六题实体零命中，audit 随之 blocked）；改写意图由 E 以正确编码重做，或由 C 将 6 题转入拒答专项口径；期间不开正式 `--with-metrics` |
...
| 事项 | 归属 | 现状与影响 |
|---|---|---|
| 六题题干重做（原文已回退） | E（或 C 定拒答口径） | 6 题实体零命中使 audit 保持 blocked；重写务必 UTF-8 全程（勿经 ASCII/ANSI 转码环节） |
