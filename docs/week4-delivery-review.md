...
**当前状态（2026-09-15，合并全部 PR 后）**：`pytest tests/` **345/345 绿**；`make audit` **pass**（循环指纹 4/120）；六套索引指纹匹配、合并全程未触发重建。
...
| 级别 | 问题 | 处置 |
|---|---|---|
| **P0** | Q021/Q023/Q028/Q059/Q060/Q119 六题中文被字面 `?` 损坏（有损转码：非 ASCII 字符在写入时被替换，ASCII 完好）。改写意图可辨（消除零命中实体）但文本已废，乱码题干会污染检索与计分 | E 重写（推荐）或 D 代拟（待用户授权）；**修复前不开正式 `--with-metrics`** |
...
| 事项 | 归属 | 现状与影响 |
|---|---|---|
| PR#36 P0 六题乱码 / P1 宽区间 keyword 复核 | E（P0 可授权 D 代拟） | 修复前不开正式 `--with-metrics`，六份 void 报告不刷新 |
