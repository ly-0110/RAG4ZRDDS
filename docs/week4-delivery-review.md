...
**当前状态（2026-09-15，合并全部 PR 后）**：`pytest tests/` **345/345 绿**；六套索引指纹匹配、合并全程未触发重建。`make audit` 现为 **blocked**——P0 乱码已按用户决策修复（`questions.jsonl` 自 `b606e9f` 单文件恢复），题集回到 6 题实体零命中的已知真阳性（循环指纹保持 4/120），待 E 重写改写意图或 C 转拒答口径后重开指标闸门。
...
**验证缺口（如实）**：`make setup` 的 venv 全新安装未本机跑（需重下 2GB+）；容器形态未交付（本机无 docker，用户决策走 make 链路）。
...
| 级别 | 问题 | 处置 |
|---|---|---|
| **P0** | Q021/Q023/Q028/Q059/Q060/Q119 六题中文被字面 `?` 损坏（有损转码：非 ASCII 字符在写入时被替换，ASCII 完好）。改写意图可辨（消除零命中实体）但文本已废，乱码题干会污染检索与计分 | **已按用户决策处置（2026-09-15）**：`questions.jsonl` 自 `b606e9f` 单文件恢复（PR#36 对该文件的改动恰为这 6 行，恢复=精确还原原文，与其他修改无关），与 `b606e9f` 逐字节一致；audit 回 **blocked**（6 题 `QUESTION_TOKEN_ABSENT`，已知真阳性）。**后续**：E 以正确编码重写改写意图，或 C 将 6 题转入拒答专项集口径 |
...
| 事项 | 归属 | 现状与影响 |
|---|---|---|
| 六题改写意图重写（P0 乱码已恢复原文） | E（或 C 定拒答口径） | audit 回 blocked（6 题实体零命中）；E 重写时务必 UTF-8 全程（勿经 ASCII/ANSI 转码环节），重写后 `make audit` 重开 |
