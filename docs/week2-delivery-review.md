...
> **v1.3（2026-09-12）**：A 域两项遗留（碎块过滤、生成性能）随 PR#28 合入 develop 定稿，
> §1/§2.1 状态相应更新（实现期另修两处新缺陷，见 §2.1）。
> ※ 本稿曾随 A 的 PR#28 带入一版以"v1.2（09-07 晚）"自称、且把 B 行回退到 PR#21 之前状态的
> 修订（A 于 09-07 晚在其工作树所写）；合并裁决时已按事实恢复并升 v1.3，见 week3 review §A-3。
...
|---|---|---|---|
| A 知识工程 | semantic/hybrid 分块（#11，09-01）；两项遗留闭环 + html_loader（#28，09-12） | 方向正确；#11 基线漂移致合入即回退（R1），已代修；#28 见 week3 review | 两项遗留已闭环（§2.1，PR#28 已合入）；semantic 真实复测待 bge-m3 环境 |
| B 检索 | BM25 全链路（#17，09-07）；检索日志字段定义 `docs/retrieval-log-schema.md`（#21，09-08）；D 侧已接线 | **合格可用**，含真实产物冒烟；日志字段定义完整（含 result_count=0 无证据信号） | score 阈值 X1 追认、B1（BM25 零分过滤）追认 |
...
**遗留闭环（A 域，2026-09-07 晚交付，PR#28 合入 2026-09-12）**
